...
func ConfigMaps(ctx context.Context, params Params) error {
	...
	if !params.Instance.Spec.Agent.Disabled {
		cm := desiredConfigMap(ctx, params, params.Instance.Spec.Agent.Config, "agent")
		desired = append(desired, cm)
	}
	if !params.Instance.Spec.ClusterReceiver.Disabled {
		cm := desiredConfigMap(ctx, params, params.Instance.Spec.ClusterReceiver.Config, "cluster-receiver")
		desired = append(desired, cm)
	}
	if !params.Instance.Spec.Gateway.Disabled {
		cm := desiredConfigMap(ctx, params, params.Instance.Spec.Gateway.Config, "gateway")
		desired = append(desired, cm)
	}
